...
[M_Dynamic_Mats] = AIDC_Control(t(j),y(:,j),SP,KC,M_Dynamic_Mats,P_Dynamic_Mats) ;
...
figure(4)
subplot(311)
plot(t,CF(1,:),t,PD(1,:),'--'), grid on
xlabel('time(sec)')
ylabel('Newtons')
title('Cartesian Forces F_x');
legend('AIDC', 'PD', 'Location', 'Best')
subplot(312)
plot(t,CF(2,:),t,PD(2,:),'--'), grid on
xlabel('time(sec)')
ylabel('Newtons')
%legend('Fdy', 'IDC_y', 'PID_y', 'F_y')
title('Cartesian Forces F_y');
subplot(313)
plot(t,CF(3,:),t,PD(3,:),'--'), grid on
xlabel('time(sec)')
ylabel('N.m')
