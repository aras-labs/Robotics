...
%
function [Out] = PlotData(t,X,Struct_Param)
...
[yd(:,j),dyd(:,j)]=TP_cubic_euler(t(j),Struct_Param);
Y(:,j)    = y(1:6,j) ;
Ydot(:,j) = y(7:12,j) ;
[Kinematic_Conf] = Kinematic_Configuration(Y(:,j),Ydot(:,j),Struct_Param) ;
...
[M_Dynamic_Mats] = PD_Control(t(j),y(:,j),Struct_Param,Kinematic_Conf,M_Dynamic_Mats,P_Dynamic_Mats) ;
PD(:,j)  = M_Dynamic_Mats.PD;
JF(:,j)  = M_Dynamic_Mats.JF;
CF(:,j)  = M_Dynamic_Mats.CF;
%
%   go back to screw representation
%
phi=yd(4:6,j);
R=euler2rot(phi);
theta=rot2sc(R);
