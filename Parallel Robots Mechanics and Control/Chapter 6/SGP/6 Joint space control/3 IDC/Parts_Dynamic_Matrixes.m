...
for i = 1:SP.n
    ...
    KC.mc(:,i)*KC.s_x(:,:,i)^2 ;
    C_temp = (((-2/KC.l(:,i))*KC.mco(:,i)*KC.ldot(:,i))*eye(3,3) - ...
    (1/KC.l(:,i)^2)*SP.m2(:,i)*SP.cp2(:,i)*KC.s(:,i)*KC.x_Bdot(:,i)')*KC.s_x(:,:,i)^2 ;        
    G_temp = (KC.mg(:,i)*KC.s_x(:,:,i)^2 - ...
     SP.m2(:,i)*KC.s(:,i)*KC.s(:,i)')*SP.g ;
    
    P_Dynamic_Mats.JT(:,i) = [KC.s(:,i); KC.B_x(:,:,i)*KC.s(:,i)];
    P_Dynamic_Mats.dJT(:,i) =  [ KC.Om_x(:,:,i)*KC.s(:,i); ...
        KC.B_x(:,:,i)*KC.Om_x(:,:,i)*KC.s(:,i) - ... 
             KC.s_x(:,:,i)*KC.Omega_EF_x*KC.B(:,i)];
...
P_Dynamic_Mats.J_1 = pinv(P_Dynamic_Mats.JT');
P_Dynamic_Mats.J_T = pinv(P_Dynamic_Mats.JT);
end