...
SP.g = [0 ; 0 ; -9.81]*1 ; % Vector of  Gravity Acceleration in the Fixed Coordinate System (m/s^2)
...
SP.KP=1e6*diag([1 1 1 10 10 10]); % The proportional controller gain KP
SP.KV=1e5*diag([1 1 1 1 1 1]); % The derivative controller gain KV
...
SP.time=Fout(:,1);   
SP.Fd=Fout(:,2:7)*0; % NO disturbance or *1e4;    % 50KN disturbance!
%% Determine Time Span
SP.ts = 0 ;
SP.tf = 2 ;
