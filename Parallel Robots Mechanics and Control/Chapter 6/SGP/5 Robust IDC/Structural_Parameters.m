...
SP.KP=1e3*diag([1 1 1 10 10 10]); % The proportional controller gain KP
SP.KV=1e2*diag([1 1 1 1 1 1]); % The derivative controller gain KV
SP.pert=  0.5; % Parameter perturbation %10

% Robust control Parameters

SP.MA=[zeros(6,6) eye(6,6); -SP.KP -SP.KV];  % Book eq. (6.27)
SP.MB=[zeros(6,6); eye(6,6)];                % Book eq. (6.27)   
SP.MQ=eye(12,12);                              % Book eq. (6.28)
SP.MP=lyap(SP.MA,SP.MQ);                     % Book eq. (6.28)

% The Robust controller gain must be high enough to compensate
% for the model uncertainties, therefore, the parameter perturbations 
% is used to adjust for the high gain
    
SP.RGain = 1e3 * (1- SP.pert);               % Robust Gain: Book eq. (6.31)
SP.eps   = 8e1 ;                             % Chattering threshold
%% Noise Amplitude
SP.noise_x  = 1e-3*[1;1;1;1*deg2rad;1*deg2rad;1*deg2rad]*0;  % no noise
SP.noise_dx = 1e-4*[1;1;1;1;1;1]*0;
...
SP.time=Fout(:,1);   
SP.Fd=Fout(:,2:7)*0;%1e4;    % 50KN disturbance!
