...
%% Desired Force Trajectory and Controller Parameters
...
%         ];
%   The desired trajectory initial points
%     Time   Fx   Fy   Fz    \tau_x   \tau_y   \tau_z 
SP.xd=[
        0       0       0        0        1       1       1      0        
        1.0    500      500     500       1       1       1      0   
        2.0    500      500     500       1       1       1      0   
        10.0   250      750     500       1       1       1      0   
        10       0       0        1       1       1       1      0   
        ];
...
SP.Ke = 1e2*diag([1 1 1 0 0 0]);   % Environment stiffness coefficient
SP.Ce = 1e1*diag([1 1 1 0 0 0]);   % Environment damping coefficient


%% Controller Parameters

SP.Kp=5e3*diag([1 1 1 1 1 1]); % The desired impedance stiffness matrix
SP.Kd=50*diag([1 1 1 1 1 1]); % The desired impedance damping matrix
SP.Kf=0.01*diag([1 1 1 1 1 1]); % The desired impedance Mass matrix
SP.Ki=1*diag([1 1 1 1 1 1]); % The desired impedance Mass matrix
SP.pert=  0.5 ; % Parameter perturbation %10

%% Noise Amplitude
SP.noise_x  = 1e-3*[1;1;1;1*deg2rad;1*deg2rad;1*deg2rad]*0;  % no noise
SP.noise_dx = 1e-3*[1;1;1;1;1;1]*0;
SP.noise_F  = 5e-1*[1;1;1;0;0;0]*0;               % Force    measurement noise
...
SP.time=Fout(:,1);   
SP.Fd=Fout(:,2:7)*1e5*0; %1e4;    % 10KN disturbance!
%% Determine Time Span
SP.ts = 0 ;
SP.tf = 1.0 ;
